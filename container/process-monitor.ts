...
export class ProcessMonitor extends EventEmitter {
	...
	constructor(
		...
		this.simpleLogManager = new SimpleLogManager(
	...
	public async start(): Promise<Result<ProcessInfo>> {
		try {
			if (this.state === 'running') {
				return {
					success: false,
					error: new Error('Process is already running'),
				};
			}
			...
			await this.simpleLogManager
				.appendLog(
					`Process started: ${this.processInfo.command} ${this.processInfo.args?.join(' ') || ''}`,
					'stdout',
				)
				.catch(() => {});
	...
	private shouldRestartAfterExit(
		...
		if (exitCode === 0) {
			const timeSinceLastActivity =
				Date.now() - this.lastActivity.getTime();
	...
	private scheduleRestart(): void {
		...
		this.restartTimer = setTimeout(async () => {
			...
			if (!result.success) {
				console.error(
					`Failed to restart process: ${result.error?.message}`,
				);
		...
		this.healthCheckTimer = setInterval(() => {
			if (this.state === 'running') {
				...
				if (
					...
					this.options.healthCheckInterval * 2
				) {
					console.warn(
						`Process appears unresponsive (no activity for ${timeSinceLastActivity}ms)`,
					);
