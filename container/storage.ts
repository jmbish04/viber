...
class ErrorStorage {
	...
	public storeError(
      ...
      `,
        ...
        `,
			...
			return this.errorResult<boolean>(
				error,
				'Unknown error storing error',
			);
		}
	}
	...
	public getErrors(instanceId: string): Result<StoredError[]> {
		try {
			const errors = this.selectErrorsStmt.all(
				instanceId,
			) as StoredError[];
...
class LogStorage {
	...
	public getLogStats(instanceId: string): Result<{
      ...
      `,
			...
			for (const stat of stats) {
				...
				logsByLevel[stat.level] =
					(logsByLevel[stat.level] || 0) + stat.total;
			...
			return {
				...
				data: {
					...
					logsByStream: logsByStream as Record<
						'stdout' | 'stderr',
						number
					>,
					oldestLog,
	...
	public cleanupOldLogs(): Result<number> {
		try {
			const result = this.deleteOldLogsStmt.run(
