	...
	static async start(options: {
		...
		try {
			...
			const errorStorage: ErrorStoreOptionsType = {
				...DEFAULT_STORAGE_OPTIONS,
				maxErrors:
					options.maxErrors ?? DEFAULT_STORAGE_OPTIONS.maxErrors,
				retentionDays:
					options.retentionDays ??
					DEFAULT_STORAGE_OPTIONS.retentionDays,
			};
			...
			const logStorage: LogStoreOptionsType = {
				...
				retentionHours:
					...
					DEFAULT_LOG_STORE_OPTIONS.retentionHours,
			};
...
class ErrorCommands {
		...
		try {
			...
			const response = {
				...
				summary: {
					...
					hasMore:
						offset + paginatedErrors.length < filteredErrors.length,
				},
			};
			...
			if (options.reset) {
				if (!storage) {
					throw new Error(
						'Storage not initialized for reset operation',
					);
				}
	...
	static async clear(options: {
		...
		if (!options.confirm) {
			OutputFormatter.formatError(
				'--confirm flag required to clear errors',
			);
			process.exit(1);
		}
...
class LogCommands {
	...
	static async get(options: {
		...
		try {
			...
			if (!lockAcquired) {
				throw new Error(
					'Could not acquire file lock for log reset operation',
				);
			}
	...
	static async clear(options: {
		...
		if (!options.confirm) {
			OutputFormatter.formatError(
				'--confirm flag required to clear logs',
			);
			process.exit(1);
		}
...
async function handleProcessCommand(
	...
	switch (subcommand) {
		case 'start':
			...
			await ProcessCommands.start({
				...
				maxRestarts: args['max-restarts']
					? parseInt(String(args['max-restarts']))
					: undefined,
				restartDelay: args['restart-delay']
					? parseInt(String(args['restart-delay']))
					: undefined,
				maxErrors: args['max-errors']
					? parseInt(String(args['max-errors']))
					: undefined,
				retentionDays: args['retention-days']
					? parseInt(String(args['retention-days']))
					: undefined,
				logRetentionHours: args['log-retention-hours']
					? parseInt(String(args['log-retention-hours']))
					: undefined,
			});
			break;
		...
		default:
			OutputFormatter.formatError(
				`Unknown process subcommand: ${subcommand}`,
			);
			process.exit(1);
	}
}
	...
	switch (subcommand) {
		case 'list':
			...
			await ErrorCommands.list({
				instanceId: String(args['instance-id']),
				minLevel: args['min-level']
					? parseInt(String(args['min-level']))
					: undefined,
				maxLevel: args['max-level']
					? parseInt(String(args['max-level']))
					: undefined,
		...
		default:
			OutputFormatter.formatError(
				`Unknown error subcommand: ${subcommand}`,
			);
			process.exit(1);
	}
}
...
async function handleLogCommand(
	...
	switch (subcommand) {
		...
		default:
			OutputFormatter.formatError(
				`Unknown log subcommand: ${subcommand}`,
			);
			process.exit(1);
	}
}
