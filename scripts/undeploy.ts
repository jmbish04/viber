...
class CloudflareUndeploymentManager {
	...
	private parseArguments(): void {
		...
		if (this.allMode && !this.forceMode) {
			console.warn(
				'⚠️  Warning: "all" mode requires --force flag for safety',
			);
		...
		if (this.allMode) {
			...
			console.log(
				'ℹ️  This will preserve D1 database and dispatch namespace',
			);
		}
	}
	...
	private async deleteKVNamespaces(): Promise<void> {
		if (
			...
			this.config.kv_namespaces.length === 0
		) {
	...
	private async deleteD1Database(): Promise<void> {
		...
		if (
			...
			this.config.d1_databases.length === 0
		) {
	...
	private async deleteDispatchNamespace(): Promise<void> {
		...
		const deletePromises = this.config.dispatch_namespaces.map(
			(dispatchNs) =>
				this.execWranglerCommandAsync(
	...
	private async deleteContainers(): Promise<void> {
		...
		try {
			...
			if (
				...
				console.warn(
					`⚠️  Could not list/delete containers: ${errorMessage}`,
				);
			}
		}
	}
	...
	public async undeploy(): Promise<void> {
		...
		try {
			...
			console.error(
				'   - Ensure you have proper Cloudflare API permissions',
			);
			console.error('   - Check that wrangler is authenticated');
			console.error(
				'   - Verify resources exist before attempting deletion',
			);
			console.error(
