...
class SetupManager {
	...
	async setup(): Promise<void> {
		try {
			...
			console.error(
				'   3. Try running the script again or set up manually',
			);
			console.error(
	...
	private async promptWithDefault(
		...
		if (existingValue) {
			const maskedValue = this.maskSensitiveValue(
				question,
				existingValue,
			);
	...
	private async collectUserConfig(): Promise<void> {
		...
		for (const choice of selectedProviders) {
			if (choice === 6) {
				...
				const provider =
					providerMap[choice as keyof typeof providerMap];
				if (provider) {
	...
	private async ensureDispatchNamespace(
		...
		try {
			...
			try {
				...
				execSync(
					`wrangler dispatch-namespace create ${namespaceName}`,
					{
						stdio: 'pipe',
						env: {
							...process.env,
	...
	private parseWorkerConfiguration(): Set<string> {
		...
		try {
			...
			const envInterfaceMatch = content.match(
				/interface Env \{([\s\S]*?)\}/,
			);
	...
	private async generateDevVarsFile(): Promise<void> {
		...
		managedVars.forEach((varName) => {
			if (!setupManagedVars.has(varName) && existingVars.has(varName)) {
				workerConfigVarsToPreserve.set(
					varName,
					existingVars.get(varName)!,
				);
			}
		});
		...
		const totalPreserved =
			preservedVars.size + workerConfigVarsToPreserve.size;
		if (totalPreserved > 0) {
			console.log(
			...
			if (preservedVars.size > 0) {
				console.log(
					`   • ${preservedVars.size} custom variables preserved`,
				);
			}
		} else {
	...
	private async generateReadinessReport(
		...
		if (
			...
			issues.push(
				'Custom domain zone not found - domain routing may not work',
			);
			suggestions.push(
	...
	private displayFinalReport(
		...
		if (this.config.setupRemote && this.config.prodDomain) {
			...
			console.log(
				'   4. For production deployment, run `npm run deploy`',
			);
		}
		...
		const hasRemoteR2 = resources.r2Buckets.some(
		...
		if (!hasGoogleAI || hasOAuth || !hasRemoteR2 || isARM64) {
			...
			if (hasOAuth) {
				console.log(
					'   • OAuth credentials configured - users can now log in',
				);
				if (
					Object.keys(this.config.devVars).includes(
						'GITHUB_EXPORTER_CLIENT_ID',
					)
				) {
					console.log(
			...
			if (!hasRemoteR2) {
				console.log(
					'   • Templates deployed to local R2 for development',
				);
				console.log(
				...
				console.log(
					'   • Templates deployed to both local and remote R2',
				);
				console.log(
	...
	private async updateWorkerConfiguration(): Promise<void> {
		...
		try {
			...
			if (lastApiKeyMatch) {
				const lastApiKeyLine =
					lastApiKeyMatch[lastApiKeyMatch.length - 1];
				const insertPoint =
	...
	private async patchDockerfileForARM64(): Promise<void> {
		...
		if (!existsSync(dockerfilePath)) {
			console.warn(
				'⚠️  SandboxDockerfile not found - skipping ARM64 patching',
			);
			return;
		}
	...
	private async deployTemplates(resources: ResourceInfo): Promise<void> {
		...
		try {
			...
			if (hasRemoteR2) {
				...
				console.log(
					'📝 Note: Templates deployed to local R2 for development',
				);
				console.log(
	...
	private async setupDatabase(resources: ResourceInfo): Promise<void> {
		...
		try {
			...
			if (hasRemoteD1) {
				console.error(
					`   ${this.packageManager} run db:migrate:remote`,
				);
			}
			console.error(
