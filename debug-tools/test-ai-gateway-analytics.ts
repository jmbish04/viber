			...
			if (totalRequests) {
				...
				const errorRate =
					totalRequests.count > 0
						? (
								(erroredRequests / totalRequests.count) *
								100
							).toFixed(2)
						: '0.00';
				const cacheHitRate =
					totalRequests.count > 0
						? (
								(cachedRequests / totalRequests.count) *
								100
							).toFixed(2)
						: '0.00';
...
async function main(): Promise<void> {
	try {
		...
		for (const { name, query } of queries) {
			...
			try {
				...
				if (
					...
					(data as ProviderRequestAnalytics)?.data?.viewer
						?.accounts?.[0]?.data
				) {
					const providerData = (data as ProviderRequestAnalytics).data
						.viewer.accounts[0].data;
		...
		if (
			...
			console.log(
				'   • Use --granularity minute for minute-level precision',
			);
			console.log(
