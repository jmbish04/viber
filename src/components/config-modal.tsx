...
const getModelRecommendation = (agentAction: string) => {
	const recommendations: Record<string, string> = {
		templateSelection:
			'💡 Recommended: Fast models for quick template selection',
		...
		phaseImplementation:
			'⚡ Recommended: Strong coding models for implementation',
		...
		screenshotAnalysis:
			'👁️ Recommended: Vision-capable models for image analysis',
	};
		...
		Object.values(byokData.modelsByProvider).forEach((providerModels) => {
			providerModels.forEach((model) => {
				...
				if (!processedModels.has(modelStr)) {
					...
					const hasUserKey = hasUserKeyForModel(
						modelStr,
						byokData.providers,
					);
						...
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-xs text-text-tertiary mt-1">
									Select primary and fallback models - we'll
									use your API keys if available
								</p>
							</div>
							<Button
								variant="outline"
								...
								{selectedModelInfo.isPlatformModel &&
									...
									formData.modelName !== 'default' && (
										<div className="flex items-center gap-2 text-sm text-blue-600 bg-blue-50 dark:bg-blue-950/20 px-3 py-2 rounded-md border border-blue-200 dark:border-blue-800">
											...
											<span>
												Platform model with usage
												limits. Consider BYOK for higher
												usage.
											</span>
										</div>
									)}
								...
								{formData.modelName &&
									...
									selectedModelInfo.hasUserKey && (
										<div className="flex items-center gap-2 text-sm text-green-600 bg-green-50 dark:bg-green-950/20 px-3 py-2 rounded-md border border-green-200 dark:border-green-800">
											...
											<span>
												Using your{' '}
												{selectedModelInfo.provider} API
												key
											</span>
										</div>
									)}
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							<div className="p-4 rounded-lg border bg-blue-50/50 border-blue-200">
								...
								<p className="text-xs text-blue-700">
									Models served through our platform with
									limited quota. No API keys required.
								</p>
							</div>
							...
							<div className="p-4 rounded-lg border bg-green-50/50 border-green-200">
								...
								<p className="text-xs text-green-700">
									Your API keys are used for direct billing
									with providers. Unlimited usage based on
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
							...
							<div className="space-y-2">
								...
								{defaultConfig?.max_tokens && (
									<p className="text-xs text-text-tertiary">
										🔧 Default:{' '}
										{defaultConfig.max_tokens?.toLocaleString()}
									</p>
								)}
							</div>
