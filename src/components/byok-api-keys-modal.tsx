...
export function ByokApiKeysModal({
	...
	const [secretToDelete, setSecretToDelete] = useState<ManagedSecret | null>(
		null,
	);
	...
	const loadBYOKProviders = async () => {
		try {
			...
			if (response.success && response.data) {
				const providers = response.data.templates.map(
					templateToBYOKProvider,
				);
	...
	const loadManagedSecrets = async () => {
		try {
			...
			if (response.success && response.data) {
				...
				const managedSecrets: ManagedSecret[] = byokSecrets.map(
					(secret) => {
						const logo =
							PROVIDER_LOGOS[secret.provider] ||
							(() => (
								<div className="w-4 h-4 bg-gray-300 rounded" />
							));
	...
	const isKeyFormatValid =
		provider && apiKey && provider.validation.test(apiKey);
				...
				setManagedSecrets((prev) =>
					prev.map((secret) =>
						secret.id === secretId
							? {
									...secret,
									isActive: updatedSecret.isActive ?? false,
								}
							: secret,
					),
				);
	...
	const handleDeleteSecret = async () => {
		...
		try {
			...
			toast.success(
				`${secretToDelete.name} API key deleted successfully`,
			);
					...
					<DialogHeader>
						<DialogTitle className="flex items-center gap-2">
							...
							<span className="flex items-center gap-1 text-xs text-text-tertiary font-normal">
								via <CloudflareLogo className="h-3 w-3" /> AI
								Gateway
							</span>
						</DialogTitle>
						<DialogDescription>
							Add your API keys to use your own provider accounts
							for billing, or manage existing keys
						</DialogDescription>
					</DialogHeader>
						...
						<TabsContent value="manage" className="space-y-4">
							{loadingSecrets ? (
								...
								<div className="text-center py-8 text-text-tertiary">
									...
									<p className="text-sm">
										Add your first API key using the "Add
										Keys" tab
									</p>
								</div>
							) : (
								<div className="space-y-4">
									<div className="flex items-center justify-between">
										...
										<Badge variant="secondary">
											{
												managedSecrets.filter(
													(s) => s.isActive,
												).length
											}{' '}
											active, {managedSecrets.length}{' '}
											total
										</Badge>
									</div>
										...
										{managedSecrets.map((secret) => {
											...
											return (
												...
												>
													...
													>
														<LogoComponent
															className={`h-5 w-5 ${secret.isActive ? '' : 'opacity-60'}`}
														/>
													</div>
					...
					<DialogFooter>
						...
						{activeTab === 'add' && selectedProvider && (
							<Button
								...
								disabled={
									!apiKey || !isKeyFormatValid || isSaving
								}
								className="gap-2"
							>
								{isSaving ? (
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							{secretToDelete?.name} API key? This action cannot
							be undone.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
