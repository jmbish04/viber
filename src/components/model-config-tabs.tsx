...
const categorizeAgent = (agentKey: string): string => {
	...
	if (key.includes('template') || key.includes('selection'))
		return 'quickstart';
	if (key.includes('blueprint') || key.includes('architect'))
		return 'quickstart';
	if (
	...
	if (key.includes('suggestion') && key.includes('process'))
		return 'planning';
	if (key.includes('planning') || key.includes('plan')) return 'planning';

	if (key.includes('implementation') || key.includes('implement'))
		return 'coding';
...
export function ModelConfigTabs({
	...
	const getAgentsForTab = useCallback(
		(tabId: string) => {
			return filteredAgentConfigs.filter(
				(config) => categorizeAgent(config.key) === tabId,
			);
		},
	...
	const getCustomizedCountForTab = useCallback(
		(tabId: string) => {
			...
			return agents.filter(
				(agent) => modelConfigs[agent.key]?.isUserOverride,
			).length;
		},
	...
	const handleTestAllConfigured = async () => {
		const customizedConfigs = agentConfigs.filter(
			(config) => modelConfigs[config.key]?.isUserOverride,
		);
		...
		toast.success(
			`Tested ${customizedConfigs.length} configs: ${successCount} passed, ${errorCount} failed`,
		);
	};
		...
		<div className="space-y-6">
			...
			<div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
				<div className="flex-1">
					...
					<p className="text-sm text-text-tertiary">
						Customize AI model settings for different operations.
			...
			>
				<TabsList className="grid w-full grid-cols-5 h-12 items-center border-2 border-bg-2 dark:border-bg-4">
					{Object.values(WORKFLOW_TABS).map((tab) => {
						const Icon = tab.icon;
						const customizedCount = getCustomizedCountForTab(
							tab.id,
						);
								...
								<div className="text-sm text-text-tertiary">
									...
									{getCustomizedCountForTab(tab.id) > 0 && (
										<span className="ml-2 text-text-primary font-medium">
											({getCustomizedCountForTab(tab.id)}{' '}
											customized)
										</span>
									)}
								</div>
									...
									<div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 gap-8 auto-rows-fr">
										{agents.map((agent) => (
											<ConfigCard
												...
												agent={agent}
												userConfig={
													modelConfigs[agent.key]
												}
												defaultConfig={
													defaultConfigs[agent.key]
												}
												onConfigure={() =>
												...
												onTest={() =>
													onTestConfig(agent.key)
												}
												onReset={() =>
													onResetConfig(agent.key)
												}
												isTesting={
													testingConfig === agent.key
												}
											/>
										))}
									</div>
			...
			{selectedConfigKey && (
				<ConfigModal
					...
					agentConfig={
						agentConfigs.find((a) => a.key === selectedConfigKey)!
					}
					...
					onTest={(tempConfig) =>
						onTestConfig(selectedConfigKey, tempConfig)
					}
