...
} from '../utils/handle-websocket-message';
import {
...
import {
	initialStages as defaultStages,
	updateStage as updateStageHelper,
} from '../utils/project-stage-helpers';
...
export function useChat({
	...
	const connectionStatus = useRef<
		'idle' | 'connecting' | 'connected' | 'failed' | 'retrying'
	>('idle');
	...
	const [projectStages, setProjectStages] =
		useState<ProjectStage[]>(defaultStages);
	...
	const [cloudflareDeploymentUrl, setCloudflareDeploymentUrl] =
		useState<string>('');
	...
	const updateStage = useCallback(
		(
			...
			data: Partial<Omit<ProjectStage, 'id'>>,
		) => {
				...
				ws.addEventListener('open', () => {
					...
					if (isRetry) {
						sendMessage({
							...
							message:
								'🔌 Connection restored! Continuing with code generation...',
						});
					}
					...
					if (!disableGenerate && urlChatId === 'new') {
						logger.debug(
							'🔄 Starting code generation for new chat',
						);
			...
			try {
				if (urlChatId === 'new') {
					if (!userQuery) {
						...
						logger.error(
							'Query is required for new code generation',
						);
					...
					for await (const obj of ndjsonStream(response.stream)) {
						...
						if (obj.websocketUrl) {
							...
							logger.debug(
								'📡 Received WebSocket URL from server:',
								result.websocketUrl,
							);
						}
						if (obj.template) {
							logger.debug(
								'Received template from server:',
								obj.template,
							);
					...
					if (!response.success || !response.data) {
						logger.error('Failed to fetch existing chat:', {
							chatId: urlChatId,
		...
		async (instanceId: string) => {
			try {
				...
				if (sendWebSocketMessage(websocket, 'deploy', { instanceId })) {
					logger.debug(
						'🚀 Deployment WebSocket message sent:',
						instanceId,
					);
				...
				logger.error(
					'❌ Error sending deployment WebSocket message:',
					error,
				);
