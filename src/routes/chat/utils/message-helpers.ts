...
export function isConversationalMessage(messageId: string): boolean {
	...
	return (
		conversationalIds.includes(messageId) || messageId.startsWith('conv-')
	);
}
...
export function addOrUpdateMessage(
	...
	if (newMessage.id === 'main') {
		const mainMessageIndex = messages.findIndex(
			(m) => m.id === 'main' && m.isThinking,
		);
	...
	return messages.map((m, i) => {
		...
		if (tool.status === 'success') {
			...
			return {
				...
				toolEvents: [
					...current,
					{ name: tool.name, status: 'success', timestamp },
				],
			};
		}
		...
		return {
			...
			toolEvents: [
				...current,
				{ name: tool.name, status: tool.status, timestamp },
			],
		};
	});
}
