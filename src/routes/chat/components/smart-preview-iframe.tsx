...
export const SmartPreviewIframe = forwardRef<
	HTMLIFrameElement,
	SmartPreviewIframeProps
>(
	(
		{
			src,
		...
		useEffect(() => {
			if (shouldRefreshPreview && retryState.hasSucceeded && currentSrc) {
				console.log(
					'🔄 Auto-refreshing preview after deployment completion',
				);
				...
				setRetryState((prev) => ({
					...
					hasSucceeded: false,
					lastError:
						'Refreshing preview after deployment completion...',
				}));
		...
		const loadWithRetry = (url: string, attempt: number) => {
			if (attempt >= maxRetries) {
					...
					try {
						webSocket.send(
							JSON.stringify({
								...
								errorMessage:
									retryState.reactError ||
									'Application failed to load after multiple attempts. Possible React router error or deployment issue.',
								url: url,
						...
						console.error(
							'Failed to send runtime error message:',
							error,
						);
					}
				}
				...
				setRetryState((prev) => ({
					...
					isRetrying: false,
					lastError:
						'Preview failed to load after multiple attempts. The deployment may still be starting up.',
				...
				testFrame.onload = () => {
					...
					setTimeout(() => {
						try {
							...
							const bodyText =
								iframeDoc?.body?.textContent?.toLowerCase() ||
								'';
							const titleText =
								iframeDoc?.title?.toLowerCase() || '';
							...
							try {
								...
								if (
									...
									setTimeout(() => {
										try {
											console.log(
												'📸 Requesting server-side screenshot of preview',
											);
											...
											console.log(
												'✅ Screenshot request sent to backend',
											);
								...
								console.log(
									'Cannot access iframe content due to CORS - assuming app is working',
								);
					...
					{retryState.isRetrying ? (
						<>
							...
							<div className="text-xs text-text-primary/50">
								Preview URLs may take a moment to become
						...
						<>
							...
							<p className="text-text-primary/70 text-sm mb-6">
								The Application may not be working yet, I will
								fix it for you. Please wait...
							</p>
							<button
								onClick={() => {
									setRetryState((prev) => ({
