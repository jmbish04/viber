						...
						{logs.length === 0 ? (
							...
							>
								...
								<p className="text-xs text-gray-400 dark:text-gray-500">
									Start by running a command to see output
									here
								</p>
							</div>
						) : (
							logs.map((log) => (
								...
								>
									...
									<div className="flex-1 min-w-0">
										...
										>
											{log.type === 'command' && (
												<span className="text-[#f6821f] font-semibold mr-1">
													$
												</span>
											)}
											{log.type === 'stderr' && (
											...
											{log.type === 'warn' && (
												<span className="text-amber-500 dark:text-yellow-400 mr-1">
													⚠️
												</span>
											)}
											{log.type === 'info' && (
				...
				<div className="mt-2 flex items-center justify-between">
					...
					{commandHistory.length > 0 && (
						<div className="text-xs text-gray-400 dark:text-gray-500">
							...
							{commandHistory.length !== 1 ? 's' : ''} in history
						</div>
					)}
				</div>
			</div>
