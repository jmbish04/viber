...
const StatusLoader = ({ size = 'md', color = 'accent' }: StatusLoaderProps) => {
	...
	return (
		<Loader className={`${sizeClass} animate-spin ${colorMap[color]}`} />
	);
};
...
function StatusIcon({ status, size = 'md', className }: StatusIconProps) {
	...
	switch (status) {
		...
		case 'completed':
			return (
				<Check
					className={clsx(iconClasses, 'text-green-500', className)}
				/>
			);
		case 'error':
			return (
				<AlertCircle
					className={clsx(iconClasses, 'text-red-500', className)}
				/>
			);
		case 'active':
			return (
		...
		default:
			return (
				<div
					className={clsx(
						iconClasses,
						'bg-bg-3-foreground/40 dark:bg-bg-3-foreground/30 rounded-full',
						className,
					)}
				/>
			);
			...
			<AnimatePresence mode="wait">
				...
				{status === 'active' && (
					<motion.div
						...
						className={clsx(
							sizeClass,
							'bg-bg-4 dark:bg-bg-2 flex items-center justify-center',
						)}
					>
...
export function PhaseTimeline({
	...
	useEffect(() => {
		if (lastPhaseRef.current) {
			lastPhaseRef.current.scrollIntoView({
				behavior: 'smooth',
				block: 'end',
			});
		}
							...
							<AnimatePresence>
								{isCollapsedBarExpanded && (
									<motion.div
										...
										transition={
											commonTransitions.premiumShort
										}
			...
			>
				...
				>
					...
					{projectStages.map((stage, index) => (
						<div
							key={stage.id}
							className="flex relative w-full gap-2 pb-2.5 last:pb-0"
						>
