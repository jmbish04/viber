			...
			return (
				<div className="fixed bottom-6 right-6 z-50 p-4 bg-red-50 border border-red-200 rounded-lg shadow-lg max-w-sm">
					...
					<p className="text-sm text-red-700 mb-3">
						The debug panel encountered an error but your main
						application is still working normally.
					</p>
					<button
						onClick={() =>
...
function DebugPanelCore({ messages, onClear, chatSessionId }: DebugPanelProps) {
	...
	const [filter, setFilter] = useState<
		'all' | 'error' | 'warning' | 'info' | 'websocket'
	>('all');
	const [wsFilter, setWsFilter] = useState<
		'all' | 'generation' | 'phase' | 'file' | 'deployment' | 'system'
	>('all');
	const [searchQuery, setSearchQuery] = useState('');
	const [isGeneratingDump, setIsGeneratingDump] = useState(false);
	const [viewMode, setViewMode] = useState<'list' | 'timeline' | 'analytics'>(
		'list',
	);
	...
	const calculateOperationMetrics = (wsMessages: DebugMessage[]) => {
		try {
			...
			const phaseStarts = wsMessages.filter(
				(m) => m.messageType === 'phase_generating',
			);
			const phaseCompletes = wsMessages.filter(
				(m) => m.messageType === 'phase_implemented',
			);
			...
			phaseStarts.forEach((start) => {
				...
				if (complete) {
					operations.phaseGeneration.push(
			...
			const fileStarts = wsMessages.filter(
				(m) => m.messageType === 'file_generating',
			);
			const fileCompletes = wsMessages.filter(
				(m) => m.messageType === 'file_generated',
			);

			fileStarts.forEach((start) => {
				const complete = fileCompletes.find(
					(c) => c.timestamp > start.timestamp,
				);
				if (complete) {
					...
					try {
						...
						const charsMatch =
							content.match(/(\d+)\s*characters?/i);
			...
			const cfStarts = wsMessages.filter(
				(m) => m.messageType === 'cloudflare_deployment_started',
			);
			const cfCompletes = wsMessages.filter(
				(m) => m.messageType === 'cloudflare_deployment_completed',
			);

			cfStarts.forEach((start) => {
				const complete = cfCompletes.find(
					(c) => c.timestamp > start.timestamp,
				);
				if (complete) {
					operations.cfDeployment.push(
						complete.timestamp - start.timestamp,
					);
				}
			});
			...
			const runnerStarts = wsMessages.filter(
				(m) => m.messageType === 'phase_implemented',
			);
			const runnerCompletes = wsMessages.filter(
				(m) => m.messageType === 'deployment_completed',
			);

			runnerStarts.forEach((start) => {
				const complete = runnerCompletes.find(
					(c) => c.timestamp > start.timestamp,
				);
				if (complete) {
					operations.runnerDeployment.push(
						complete.timestamp - start.timestamp,
					);
				}
			});
			...
			const getStats = (durations: number[]) => {
				if (durations.length === 0)
					return { avg: 0, median: 0, p99: 0, count: 0 };
			...
			const getFileStats = (
				fileOps: { duration: number; lines: number; chars: number }[],
			) => {
				if (fileOps.length === 0) {
					return {
				...
				const linesPerSec = fileOps.map((op) =>
					op.duration > 0 ? op.lines / (op.duration / 1000) : 0,
				);
				const charsPerSec = fileOps.map((op) =>
					op.duration > 0 ? op.chars / (op.duration / 1000) : 0,
				);

				const totalLines = fileOps.reduce(
					(sum, op) => sum + op.lines,
					0,
				);
				const totalChars = fileOps.reduce(
					(sum, op) => sum + op.chars,
					0,
				);
	...
	const processTimelineData = (messages: DebugMessage[]) => {
		try {
			...
			const events = messages.map((msg, index) => ({
				...
				category:
					msg.wsCategory ||
					categorizeWebSocketMessage(msg.messageType),
				duration:
					index > 0
						? (msg.timestamp || 0) -
	...
	const analyticsData = useMemo(() => {
		try {
				...
				sortedIntervals.length > 0
					? sortedIntervals.length % 2 === 0
						? (sortedIntervals[sortedIntervals.length / 2 - 1] +
							...
							2
						: sortedIntervals[
								Math.floor(sortedIntervals.length / 2)
							]
					: 0;
			...
			const p99 =
				sortedIntervals.length > 0
					? sortedIntervals[Math.max(0, p99Index)]
					: 0;
			const avgInterval =
				intervals.length > 0
			...
			return {
				...
				warningRate:
					messages.length > 0
						? ((warnings.length / messages.length) * 100).toFixed(1)
						: '0',
	...
	const categorizeWebSocketMessage = (
		messageType?: string,
	):
		| 'generation'
		| 'phase'
	...
	const filteredMessages = messages.filter((msg) => {
		...
		if (filter === 'websocket' && wsFilter !== 'all') {
			const category =
				msg.wsCategory || categorizeWebSocketMessage(msg.messageType);
					...
					<div className="flex gap-1 flex-wrap">
						{[
							...
							},
							{
								key: 'info' as const,
								label: 'Info',
								count: messages.filter((m) => m.type === 'info')
									.length,
							},
						].map(({ key, label, count }) => (
							<button
							...
							{analyticsData ? (
								<>
									...
									<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
										...
										<div className="bg-purple-50 p-4 rounded-lg">
											...
											<div className="text-lg font-bold text-purple-600">
												{analyticsData.wsMessages}{' '}
												messages
											</div>
											<div className="text-sm text-purple-700">
												Last 24h:{' '}
												{analyticsData.last24h} messages
											</div>
										</div>
									...
									<div className="space-y-6">
										...
										{analyticsData.operations.fileGeneration
											.duration.count > 0 && (
											<div className="bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-200 rounded-xl p-6">
												<div className="flex items-center gap-3 mb-4">
													<div className="w-3 h-3 bg-purple-500 rounded-full"></div>
													<h5 className="font-bold text-purple-900 text-lg">
														📝 File Generation
														Performance
													</h5>
												</div>
												...
												<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
													<div className="bg-bg-4/70 dark:bg-bg-4/50 rounded-lg p-4 text-center">
														<div className="text-2xl font-bold text-purple-600">
															{analyticsData.operations.fileGeneration.linesPerSecond.avg.toFixed(
																1,
															)}
														</div>
												...
												<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
													<div className="bg-bg-4/70 dark:bg-bg-4/50 rounded-lg p-4">
														<h6 className="font-medium text-text-primary mb-2">
															⚡ Generation Speed
														</h6>
														<div className="space-y-1 text-sm">
															<div>
															...
															<div>
																Median:{' '}
																<span className="font-mono font-medium">
																	{analyticsData.operations.fileGeneration.linesPerSecond.median.toFixed(
																		1,
																	)}{' '}
																	lines/s
																</span>
															...
															<div>
																Peak (P99):{' '}
																<span className="font-mono font-medium">
																	{analyticsData.operations.fileGeneration.linesPerSecond.p99.toFixed(
																		1,
																	)}{' '}
																	lines/s
																</span>
															...
															0 ? (
																<div className="space-y-3">
																	<div className="text-center">
																		<div
																			className={`text-2xl font-bold text-${operationConfig.color}-600`}
																		>
																			{simpleStats.avg >
																			1000
																		...
																		<div
																			className={`text-sm text-${operationConfig.color}-800`}
																		>
																			Average
																			Duration
																		</div>
							...
							{bookmarkedMessages.size > 0 && (
								<div className="bg-amber-50 p-4 rounded-lg">
									...
									<div className="space-y-2">
										{messages
											.filter((m) =>
												bookmarkedMessages.has(m.id),
											)
											.slice(0, 5)
											.map((msg) => (
												<div
													key={msg.id}
													className="text-sm text-amber-700 truncate"
												>
						...
						<div className="p-4">
							{timelineData && timelineData.events.length > 0 ? (
								<div className="space-y-6">
									...
									<div className="flex flex-wrap gap-2 mb-4">
										{timelineData.lanes.map((lane) => (
											<div
												key={lane.id}
												className={`px-3 py-1 rounded text-xs font-medium ${lane.color}`}
											>
												{lane.label}
											</div>
										))}
									...
									<div className="relative">
										...
										<div className="space-y-4">
											{timelineData.events.map(
												(event, index) => {
													const lane =
														timelineData.lanes.find(
															(l) =>
																l.id ===
																event.category,
														) ||
														timelineData.lanes[4]; // Default to system
													const relativeTime =
														index > 0
															? event.timestamp -
																timelineData
														...
														>
															...
															<div className="ml-4 flex-1 min-w-0">
																<div className="flex items-center justify-between">
																	<div className="flex items-center gap-2">
																		<span
																			className={`px-2 py-1 text-xs font-medium rounded ${lane.color}`}
																		>
																			{
																				lane.label
																			}
																...
																<div
																	className="mt-2 text-sm text-text-primary truncate"
																	title={
																		event.message
																	}
																>
								...
								<div className="text-center py-12 text-text-tertiary">
									...
									<p className="text-xs text-text-tertiary/70 mt-2">
										Messages will appear here as they are
										logged
									</p>
								</div>
							)}
