...
const getProviderInfo = (modelValue?: string) => {
	if (!modelValue)
		return { name: 'Default', color: 'bg-bg-3 text-text-tertiary' };
	...
	if (
		modelValue.includes('google-ai-studio/') ||
		...
		<div className="p-4 border rounded-lg bg-bg-3/50 space-y-3">
			<div className="flex items-start justify-between gap-2">
				<div className="flex items-start gap-2 min-w-0 flex-1">
					...
					<div className="min-w-0 flex-1">
						<h6
							className="font-medium text-sm mb-1 text-text-secondary"
							title={agent.name}
						>
							{agent.name}
						</h6>
						<p
							className="text-xs text-text-tertiary line-clamp-2"
			...
			<div className="space-y-2">
				<div className="flex items-center justify-between gap-2">
					...
					<Badge
						variant="secondary"
						className={`text-xs shrink-0 ${providerInfo.color}`}
					>
						{providerInfo.name}
					</Badge>
				</div>
...
export function ModelConfigInfo({
	configs,
	onRequestConfigs,
	loading,
}: ModelConfigInfoProps) {
	...
	const getAgentsForTab = (tabId: string) => {
		if (!configs) return [];
		return configs.agents.filter(
			(agent) => categorizeAgent(agent.key) === tabId,
		);
	};
	...
	const getCustomizedCountForTab = (tabId: string) => {
		...
		return agents.filter(
			(agent) => configs?.userConfigs[agent.key]?.isUserOverride,
		).length;
	};
			...
			<Dialog open={isOpen} onOpenChange={setIsOpen}>
				<DialogContent className="max-w-4xl w-[90vw] max-h-[90vh] overflow-y-auto">
					<DialogHeader>
						...
						<DialogDescription>
							View the AI model settings currently being used for
							app generation (defaults + your overrides).
						</DialogDescription>
					</DialogHeader>
