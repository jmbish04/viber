...
export class DORateLimitStore extends DurableObject<Env> {
	...
	async increment(
		...
		const mainBuckets = this.getBucketsInWindow(
			key,
			now,
			mainWindow,
			bucketSize,
		...
		const burstBuckets = config.burst
			? this.getBucketsInWindow(key, now, burstWindow, bucketSize)
			: [];
	...
	async resetLimit(key?: string): Promise<void> {
		...
		if (key) {
			...
			const keysToDelete = Array.from(this.state.buckets.keys()).filter(
				(bucketKey) => bucketKey.startsWith(`${key}:`),
			);
