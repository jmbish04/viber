...
export async function fixModuleNotFound(
	...
	for (const issue of issues) {
		...
		try {
			...
			logger.info(
				`Finding import at line ${issue.line} in ${issue.filePath}`,
			);
			...
			logger.info(
				`Module file search result: ${foundFile || 'NOT FOUND'}`,
			);
			...
			if (foundFile) {
				...
				const relativeImport = makeRelativeImport(
					issue.filePath,
					foundFile,
				);
				logger.info(
					`Generated relative import path: "${relativeImport}"`,
				);

				logger.info(`Updating AST for import path change`);
				const updatedAST = updateImportPath(
					ast,
					importInfo,
					relativeImport,
				);
				...
				logger.info(
					`Successfully fixed import path for ${issue.filePath}`,
				);
			} else {
				logger.info(
				...
				logger.info(
					`Successfully created stub file: "${targetFilePath}"`,
				);
			}
		} catch (error) {
			logger.error(
