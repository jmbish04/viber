...
export async function fixMissingExportedMember(
	...
	for (const issue of issues) {
		...
		try {
			...
			const moduleSpecifier = extractModuleSpecifierFromMessage(
				issue.message,
			);
			...
			const { sourceAST, importInfo, targetFilePath, targetAST } =
				filesResult;
			logger.info(
			...
			logger.info(
				`Analyzing usage of ${missingExportName} in source file`,
			);
			const usageAnalysis = analyzeImportUsage(sourceAST, [
				missingExportName,
			]);
			...
			const modifiedTargetAST = addExportToFile(
				...
				exportUsage
					? {
							...
							properties: exportUsage.properties as
								| string[]
								| undefined,
							parameters:
								(exportUsage.parameters as unknown[])?.map(
									(p) => String(p),
								) || undefined,
						}
					: undefined,
	...
	if (usageAnalysis) {
		switch (usageAnalysis.type) {
			...
			default:
				...
				exportDeclaration = createVariableExport(
					exportName,
					stubComment,
				);
		}
