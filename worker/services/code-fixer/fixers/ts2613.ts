...
export async function fixModuleIsNotModule(
	...
	for (const issue of issues) {
		...
		try {
			...
			const namespaceImport = findNamespaceImportAtLocation(
				ast,
				issue.line,
			);
			...
			logger.info(
				`Files in context: ${Array.from(context.files.keys()).join(', ')}`,
			);
			...
			if (fixed) {
				...
				logger.info(
					`Successfully fixed TS2613 issue for ${issue.filePath}`,
				);
			} else {
				logger.warn(
					...
					if (
						...
						path.node.specifiers = [
							t.importDefaultSpecifier(
								t.identifier(namespaceImport.namespace),
							),
						];
						changes.push(
