	...
	for (const [filePath, fileIssues] of issuesByFile) {
		try {
			...
			for (const issue of fileIssues) {
				...
				if (!parseResult) {
					logger.warn(
						`Could not parse TS2724 error message: ${issue.message}`,
					);
				...
				if (!importInfo) {
					logger.warn(
						`No import found at line ${issue.line} in ${filePath}`,
					);
				...
				if (targetFile) {
					...
					if (targetAST) {
						...
						logger.warn(
							`Could not parse target file: ${targetFile}`,
						);
						unfixableIssues.push(
							createUnfixableIssue(
								issue,
								'Could not parse target module file',
							),
						);
					}
				} else {
			...
			if (replacementsByImport.size > 0) {
				...
				if (fixedAST) {
					...
					for (const issue of processedIssues) {
						const parseResult = parseTS2724ErrorMessage(
							issue.message,
						);
						if (parseResult) {
							fixedIssues.push({
