		...
		ExportNamedDeclaration(path) {
			...
			if (path.node.source) {
				...
				if (t.isVariableDeclaration(path.node.declaration)) {
					for (const declarator of path.node.declaration
						.declarations) {
	...
	if (fileFetcher && fetchedFiles && !fetchedFiles.has(filePath)) {
		try {
			...
			if (result && isScriptFile(result.filePath)) {
				logger.info(
					`ImportUtils: Successfully fetched ${filePath}, storing in files map`,
				);
				...
				logger.info(
					`ImportUtils: File ${filePath} was fetched but is not a script file or result is null`,
				);
			}
		} catch (error) {
			logger.warn(
		...
		logger.info(
			`ImportUtils: Not fetching ${filePath} - fileFetcher: ${!!fileFetcher}, fetchedFiles: ${!!fetchedFiles}, alreadyFetched: ${fetchedFiles?.has(filePath)}`,
		);
	}
	...
	traverseAST(ast, {
		ImportDeclaration(path) {
			if (
				...
				if (
					namespaceImport &&
					t.isImportNamespaceSpecifier(namespaceImport)
				) {
					...
					if (
						...
						newSpecifiers.push(
							t.importDefaultSpecifier(namespaceImport.local),
						);
						changes.push(
					...
					for (const namedImport of namedImports) {
						if (
							...
							if (
								exports.namedExports.includes(namedImportName)
							) {
								// Valid named export, keep it
								newSpecifiers.push(namedImport);
							} else if (
								exports.defaultExport === namedImportName
							) {
								// This should be a default import
								if (
									!newSpecifiers.some((s) =>
										t.isImportDefaultSpecifier(s),
									)
								) {
				...
				if (fixed && newSpecifiers.length > 0) {
					path.node.specifiers =
						newSpecifiers as typeof path.node.specifiers;
				}
			}
		},
	});
