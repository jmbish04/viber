...
export class GitHubService {
	...
	static async createUserRepository(
		...
		try {
			...
			GitHubService.logger.info(
				`Successfully created repository: ${repository.html_url}`,
			);
		...
		try {
			GitHubService.logger.info('Initiating GitHub push operation', {
				...
				hasUncommittedChanges:
					gitContext?.hasUncommittedChanges || false,
			});

			const octokit = GitHubService.createOctokit(request.token);

			// Parse repository info from URL
			const repoInfo = GitHubService.extractRepoInfo(
				request.cloneUrl || request.repositoryHtmlUrl,
			);
			if (!repoInfo) {
				throw new GitHubServiceError(
			...
			const remoteCommits = await GitHubService.fetchRemoteCommits(
				octokit,
				owner,
				repo,
				defaultBranch,
			...
			const parentCommitSha =
				remoteCommits.length > 0 ? remoteCommits[0].sha : '';
			...
			const commitStrategy = GitHubService.planCommitStrategy(
				gitContext,
				remoteCommits,
				files,
			);
			...
			if (commitStrategy.commits.length === 0) {
				GitHubService.logger.info(
					'No commits needed - repository is already in sync',
				);
				return { success: true, commitSha: parentCommitSha };
			}
	...
	private static async fetchRemoteCommits(
		...
		try {
			...
			if (githubError.status === 409 || githubError.status === 404) {
				...
				GitHubService.logger.info(
					'Remote repository is empty or branch does not exist',
				);
				return [];
			}
			throw error;
	...
	private static planCommitStrategy(
		...
		const newLocalCommits = localCommits.filter(
			(local) =>
				!remoteCommits.some(
					(remote) => remote.message === local.message,
				),
		);
	...
	private static async executeCommitStrategy(
		...
		for (const commitPlan of strategy.commits) {
			if (!parentCommitSha) {
				...
				GitHubService.logger.info(
					'Bootstrapping empty repository with README',
					{
						owner,
						repo,
						branch,
				...
				if (!readmeCommit.commit.sha) {
					throw new GitHubServiceError(
						'Failed to get commit SHA from README creation',
						'COMMIT_SHA_MISSING',
					);
				}
			...
			const { data: commit } = await octokit.rest.git.createCommit({
				...
				author: GitHubService.createAuthorInfo(
					request,
					commitPlan.timestamp,
				),
				committer: GitHubService.createAuthorInfo(
					request,
					commitPlan.timestamp,
				),
			});
