...
export class SandboxSdkClient extends BaseSandboxService {
	...
	private async executeCommand(
		...
		return await this.getSandbox().exec(`cd ${instanceId} && ${command}`, {
			timeout,
		});
	...
	private async getInstanceMetadata(
		...
		try {
			const metadataFile = await this.getSandbox().readFile(
	...
	private async storeInstanceMetadata(
		...
		await this.getSandbox().writeFile(
			this.getInstanceMetadataFile(instanceId),
	...
	private async checkTemplateExists(templateName: string): Promise<boolean> {
		...
		const checkResult = await sandbox.exec(
			`test -f ${templateName}/package.json && echo "exists" || echo "missing"`,
		);
		return (
			checkResult.exitCode === 0 && checkResult.stdout.trim() === 'exists'
		);
	}
	...
	private async ensureTemplateExists(
		...
		if (!(await this.checkTemplateExists(templateName))) {
			...
			this.logger.info(
				`Template doesnt exist, Downloading template from: ${templateName}`,
			);

			const zipData = await this.downloadTemplate(
				templateName,
				downloadDir,
			);
			...
			await this.getSandbox().writeFile(
				`${templateName}.zip.b64`,
				base64Data,
			);

			// Convert base64 back to binary zip file
			await this.getSandbox().exec(
				`base64 -d ${templateName}.zip.b64 > ${templateName}.zip`,
			);
			this.logger.info(
				`Wrote and converted zip file to sandbox: ${templateName}.zip`,
			);

			const setupResult = await this.getSandbox().exec(
				`unzip -o -q ${templateName}.zip -d ${isInstance ? '.' : templateName}`,
			);

			if (setupResult.exitCode !== 0) {
				throw new Error(
					`Failed to download/extract template: ${setupResult.stderr}`,
				);
			}
		} else {
	...
	async getTemplateDetails(
		...
		try {
			...
			const [fileTree, catalogInfo, dontTouchFiles, redactedFiles] =
				await Promise.all([
			...
			if (!fileTree) {
				throw new Error(
					`Failed to build file tree for template ${templateName}`,
				);
			}
			...
			const filesResponse = await this.getFiles(
				...
				true,
				redactedFiles,
			); // Use template name as directory
			...
			try {
				const packageJsonFile = filesResponse.files.find(
	...
	private async getTemplateFromCatalog(
		templateName: string,
	...
	private async buildFileTree(
		...
		try {
			...
			const excludedDirsFind = EXCLUDED_DIRS.map(
				(dir) => `-name "${dir}"`,
			).join(' -o ');
			...
			const excludedFilesFind = excludedFileTypes
			...
			if (filesResult.exitCode === 0) {
				...
				const fileSet = new Set(
					files.map((f) => (f.startsWith('./') ? f.substring(2) : f)),
				);
	...
	async listAllInstances(): Promise<ListInstancesResponse> {
		try {
			...
			const sections = bulkResult.stdout
				.split('===FILE:')
		...
		try {
			...
			try {
				...
				if (isReady) {
					...
					this.logger.warn(
						'Development server may not be fully ready',
						{ instanceId },
					);
				}
	...
	private async provisionTemplateResources(
		...
		try {
			...
			const wranglerFile = await sandbox.readFile(
				`${instanceId}/wrangler.jsonc`,
			);
	...
	private async updateProjectConfiguration(
		...
		try {
			...
			const packageJsonResult = await sandbox.exec(
				`cd ${instanceId} && sed -i '1,10s/^[ \t]*"name"[ ]*:[ ]*"[^"]*"/  "name": "${projectName}"/' package.json`,
			);
			...
			const wranglerResult = await sandbox.exec(
				`cd ${instanceId} && sed -i '0,/"name":/s/"name"[ ]*:[ ]*"[^"]*"/"name": "${projectName}"/' wrangler.jsonc`,
			);
	...
	private async setupInstance(
		instanceId: string,
		projectName: string,
		_localEnvVars?: Record<string, string>,
	): Promise<
		| {
				previewURL: string;
				tunnelURL: string;
	...
	> {
		try {
			...
			try {
				const wranglerConfigFile = await sandbox.readFile(
					`${instanceId}/wrangler.jsonc`,
				);
	...
	private async fetchDontTouchFiles(templateName: string): Promise<string[]> {
		...
		try {
			...
			const donttouchFile = await this.getSandbox().readFile(
				`${templateName}/.donttouch_files.json`,
			);
			if (donttouchFile.exitCode !== 0) {
				this.logger.warn(
					`Failed to read .donttouch_files.json: ${donttouchFile.content}`,
				);
			}
			donttouchFiles = JSON.parse(donttouchFile.content) as string[];
		} catch (error) {
			this.logger.warn(
				`Failed to read .donttouch_files.json: ${error instanceof Error ? error.message : 'Unknown error'}`,
			);
		}
	...
	private async fetchRedactedFiles(templateName: string): Promise<string[]> {
		...
		try {
			...
			const redactedFile = await this.getSandbox().readFile(
				`${templateName}/.redacted_files.json`,
			);
			if (redactedFile.exitCode !== 0) {
				this.logger.warn(
					`Failed to read .redacted_files.json: ${redactedFile.content}`,
				);
			}
			redactedFiles = JSON.parse(redactedFile.content) as string[];
		} catch (error) {
			this.logger.warn(
				`Failed to read .redacted_files.json: ${error instanceof Error ? error.message : 'Unknown error'}`,
			);
		}
	...
	async createInstance(
		...
		try {
			...
			const moveTemplateResult = await this.getSandbox().exec(
				`mv ${templateName} ${instanceId}`,
			);
			if (moveTemplateResult.exitCode !== 0) {
				throw new Error(
					`Failed to move template: ${moveTemplateResult.stderr}`,
				);
			}

			const setupPromise = () =>
				this.setupInstance(instanceId, projectName, localEnvVars);
			...
			this.logger.error('createInstance', error, {
				templateName: templateName,
				projectName: projectName,
			});
			return {
		...
		try {
			...
			try {
				...
				if (metadata.processId) {
					try {
						const process = await this.getSandbox().getProcess(
							metadata.processId,
						);
	...
	async shutdownInstance(instanceId: string): Promise<ShutdownResponse> {
		try {
			...
			if (metadata.allocatedPort) {
				...
				try {
					const exposedPorts =
						await sandbox.getExposedPorts('localhost');
	...
	async writeFiles(
		...
		try {
			...
			const writePromises = filteredFiles.map((file) =>
				sandbox.writeFile(
					`${instanceId}/${file.filePath}`,
					file.fileContents,
				),
			);
			...
			// Try to commit
			try {
				const commitResult = await this.createLatestCommit(
					instanceId,
					commitMessage || 'Initial commit',
				);
				this.logger.info('Files committed to git', {
					result: commitResult,
				});
			} catch (error) {
	...
	async getFiles(
		...
		try {
			...
			if (!filePaths) {
				...
				const importantFiles = await sandbox.exec(
					`cd ${templateOrInstanceId} && jq -r '.[]' .important_files.json | while read -r path; do if [ -d "$path" ]; then find "$path" -type f; elif [ -f "$path" ]; then echo "$path"; fi; done`,
				);
				this.logger.info(
		...
		try {
			...
			for (const command of commands) {
				try {
					...
					if (
						result.exitCode === 2 &&
						result.stderr.includes("/bin/sh: 1: cd: can't cd to i-")
					) {
	...
	async clearInstanceErrors(
		...
		try {
			...
			this.logger.info(
				`Cleared ${clearedCount} errors for instance ${instanceId}`,
			);
	...
	async fixCodeIssues(
		...
		try {
			...
			const fileFetcher: FileFetcher = async (filePath: string) => {
				...
				try {
					const result = await this.getSandbox().readFile(
						`${instanceId}/${filePath}`,
					);
					if (result.success) {
						this.logger.info(
	...
	async deployToCloudflareWorkers(
		...
		try {
			...
			if (!workerFile.success) {
				throw new Error(
					`Worker script not found at ${workerPath}. Please build the project first.`,
				);
			}
			...
			try {
				...
				const workerAssetsResult = await sandbox.exec(
			...
			const assetDirResult = await sandbox.exec(
	...
	private async createLatestCommit(
		instanceId: string,
		commitMessage: string,
	): Promise<string> {
		...
		if (statusResult.exitCode !== 0) {
			...
			const hashResult = await this.executeCommand(
				instanceId,
				`git rev-parse HEAD`,
			);
			...
			throw new Error(
				`No commits found in repository: ${hashResult.stderr}`,
			);
		}
		...
		if (addResult.exitCode !== 0) {
			...
			const altAddResult = await this.executeCommand(
				instanceId,
				`git add . 2>/dev/null || git add --all`,
			);
			if (altAddResult.exitCode !== 0) {
				throw new Error(
					`Git add failed: ${addResult.stderr || altAddResult.stderr}`,
				);
			}
		}
		...
		const commitResult = await this.executeCommand(
			instanceId,
			`git commit -m "${sanitizedMessage}" --allow-empty-message`,
		);
	...
	async exportToGitHub(
		...
		try {
			this.logger.info(
				`Starting GitHub export for instance ${instanceId}`,
			);
			...
			const createResult = await GitHubService.createUserRepository({
				...
				description:
					request.description ||
					`Generated app: ${request.repositoryName}`,
			...
			this.logger.info(
				`Repository created: ${createResult.repository.html_url}`,
			);
	...
	async pushToGitHub(
		...
		try {
			...
			if (
				gitContext.hasUncommittedChanges ||
				gitContext.hasUntrackedFiles
			) {
				...
				try {
					...
					await this.createLatestCommit(
						instanceId,
						'Auto-commit before GitHub push',
					);
			...
			const filesToUse =
				finalGitContext.allFiles.length > 0
					? finalGitContext.allFiles
					: finalGitContext.trackedFiles;
			...
			const result = await GitHubService.pushFilesToRepository(
				files,
				request,
				{
					localCommits: finalGitContext.localCommits,
					hasUncommittedChanges:
						finalGitContext.hasUncommittedChanges,
				},
			);
			...
			this.logger.error('pushToGitHub failed', error, {
				instanceId,
				repositoryUrl: request.repositoryHtmlUrl,
			});
	...
	private async extractGitContext(instanceId: string): Promise<{
		...
		try {
			...
			const logResult = await this.executeCommand(
				instanceId,
				'git log --oneline --reverse --pretty=format:"%H|%s|%cI"',
			);
			const localCommits: Array<{
				hash: string;
				message: string;
				timestamp: string;
			}> = [];
				...
				for (const line of commitLines) {
					...
					if (hash && message) {
						localCommits.push({
							...
							timestamp:
								timestamp?.trim() || new Date().toISOString(),
						});
					}
				}
			}
			...
			const lsFilesResult = await this.executeCommand(
				instanceId,
				'git ls-files',
			);
			const trackedFiles =
				lsFilesResult.exitCode === 0
			...
			const untrackedResult = await this.executeCommand(
				instanceId,
			...
			this.logger.warn(
				'Failed to extract git context, using defaults',
				error,
			);
			return {
				localCommits: [],
	 ...
	 */
	private async getGitTrackedFiles(
		instanceId: string,
		filePaths: string[],
	): Promise<
		{
	...
	> {
		...
		this.logger.info(`Reading ${filePaths.length} files for GitHub push`, {
			instanceId,
		});

		for (const filePath of filePaths) {
			try {
				const readResult = await this.getSandbox().readFile(
					`${instanceId}/${filePath}`,
				);
				if (readResult.success && readResult.content) {
					...
					this.logger.debug(`Successfully read file: ${filePath}`, {
						sizeBytes: readResult.content.length,
					});
				} else {
		...
		this.logger.info(
			`Successfully read ${files.length}/${filePaths.length} files for GitHub push`,
		);
		return files;
	}
}
