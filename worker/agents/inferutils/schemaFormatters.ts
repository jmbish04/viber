...
function formatZodSchemaAsMarkdownFields(
	...
	for (const key of Object.keys(shape)) {
		...
		if (debug)
			logger.debug(
				`[Format Data Path: object.${key}] Processing key "${key}"`,
			);
...
function formatZodTypeAsMarkdown(
	...
	if (!hasValue) {
		...
		if (
			baseField instanceof z.ZodArray ||
			baseField instanceof z.ZodObject
		) {
			return '';
		}
...
function getSimpleTypeDescription(field: z.ZodTypeAny): string {
	...
	if (
		...
		return (
			getSimpleTypeDescription(field._def.innerType) +
			' (optional/nullable)'
		);
	}
	return 'value';
...
function convertToPrimitive(
	...
	if (schema instanceof z.ZodString) {
		...
		if (debug)
			currentLogger.debug(
				`[Convert Path: ${pathStr}] Parsed number: ${num}`,
			);
		...
		if (['true', 'yes', '1', 'on'].includes(lower)) {
			...
			if (debug)
				currentLogger.debug(
					`[Convert Path: ${pathStr}] Parsed boolean: true`,
				);
			return true;
		}
		if (['false', 'no', '0', 'off'].includes(lower)) {
			...
			if (debug)
				currentLogger.debug(
					`[Convert Path: ${pathStr}] Parsed boolean: false`,
				);
			return false;
		}
		// This case should ideally be caught by ZodOptional/Nullable/Default handling above
		...
		const matchedEnum = enumValues.find(
			(enumVal) =>
				...
				enumVal.toLowerCase() === lowerValue,
		);
...
function preprocessMarkdown(markdown: string, debug: boolean): string {
	...
	if (match && match[1]) {
		...
		if (match[1].includes('##')) {
			if (debug)
				logger.debug(
					'Removed outer ```...``` fences (heuristic match).',
				);
	...
	if (firstHeadingIndex > 0) {
		...
		if (!preamble.includes('\n##')) {
			if (debug)
				logger.debug(
					'Removing potential preamble before first H2 heading.',
				);
...
function getSemanticSimilarity(
	...
	for (let n = config.ngramMinSize; n <= config.ngramMaxSize; n++) {
		...
		if (config.weights) {
			const weightKey =
				n === 1 ? 'unigram' : n === 2 ? 'bigram' : `ngram${n}`;
			const weight =
				config.weights[weightKey] ||
				(n === config.ngramMaxSize ? 1 : 0); // Default to weight 1 for max N-gram if not specified
...
function mapSectionToSchema(
	...
	if (baseSchema instanceof z.ZodObject) {
		if (debug)
			logger.debug(`[Map Path: ${currentPath}] Handling ZodObject.`);
		...
		if (debug)
			logger.debug(
				`[Map Path: ${currentPath}] Finished processing children`,
				{
					path: currentPath,
					outputObject,
				},
	...
	if (baseSchema instanceof z.ZodArray) {
		if (debug)
			logger.debug(`[Map Path: ${currentPath}] Handling ZodArray.`);
		...
		if (!(itemSchema instanceof z.ZodObject)) {
			...
			const contentString = extractPrimitiveValueFromNodes(
				section.nodes,
				{
					path: currentPath,
