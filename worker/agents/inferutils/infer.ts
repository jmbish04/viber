...
interface InferenceParamsStructured<T extends z.AnyZodObject>
...
export async function executeInference<T extends z.AnyZodObject>({
	...
	if (modelConfig) {
		...
		if (conf) {
			logger.info(
				`Using user configuration for ${agentActionName}: ${JSON.stringify(conf)}`,
			);
		} else {
			logger.info(
				`No user configuration for ${agentActionName}, using AGENT_CONFIG defaults`,
			);
		}
	}
		...
		try {
			...
			const result = schema
				? await infer<T>({
						...
						tools,
						stream,
						reasoning_effort: useCheaperModel
							? undefined
							: reasoning_effort,
						temperature,
					})
			...
			if (error instanceof InferError) {
				...
				if (
					...
					error.partialResponse.length > 1000
				) {
					messages.push(
						createAssistantMessage(error.partialResponse),
					);
					messages.push(
