...
function accumulateToolCallDelta(
	...
	if (idFromDelta && byId.has(idFromDelta)) {
		...
		console.log(
			`[TOOL_CALL_DEBUG] Found existing entry by id: ${idFromDelta}`,
		);
...
async function executeToolCalls(
	...
	const toolDefinitions = new Map(
		originalDefinitions.map((td) => [td.function.name, td]),
	);
	return Promise.all(
		openAiToolCalls.map(async (tc) => {
			try {
				...
				console.log(
					`Tool execution result for ${tc.function.name}:`,
					result,
				);
				return {
					id: tc.id,
				...
				console.error(
					`Tool execution failed for ${tc.function.name}:`,
					error,
				);
				return {
					id: tc.id,
...
export async function infer<OutputSchema extends z.AnyZodObject>(
	...
	if (messages.length > 100) {
		throw new RateLimitExceededError(
			'There is a limit of 100 messages per inference',
	...
	try {
		...
		const { apiKey, baseURL, defaultHeaders } =
			await getConfigurationForModel(modelName, env, metadata.userId);
		...
		console.log(
			`Running inference with ${modelName} using structured output with ${format} format, reasoning effort: ${reasoning_effort}, max tokens: ${maxTokens}, temperature: ${temperature}, baseURL: ${baseURL}`,
		);
		...
		try {
			...
			response = await client.chat.completions.create(
				{
					...
					model: modelName,
					messages:
						messagesToPass as OpenAI.ChatCompletionMessageParam[],
			...
			if (
				...
				throw new RateLimitExceededError(
					'Rate limit exceeded in LLM calls, Please try again later',
					RateLimitType.LLM_CALLS,
				);
			}
			throw error;
		...
		if (stream) {
			...
			if (response instanceof Stream) {
				...
				for await (const event of response) {
					const delta = (event as ChatCompletionChunk).choices[0]
						?.delta;
					...
					const finishReason = (event as ChatCompletionChunk)
						.choices[0]?.finish_reason;
					if (
						slice.length >= stream.chunk_size ||
						finishReason != null
					) {
		...
		try {
			...
			if (!result.success) {
				...
				throw new Error(
					`Failed to validate AI response against schema: ${result.error instanceof Error ? result.error.message : String(result.error)}`,
				);
			}
