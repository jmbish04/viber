...
export class SCOFFormat extends CodeGenerationFormat {
	...
	private processLine(
		...
		if (scofState.parsingMultiLineCommand) {
			...
			const multiLineCommand = this.tryParseCommand(
				scofState.commandBuffer,
			);
			if (multiLineCommand) {
			...
			const immediateCommand = this.tryParseCommand(
				scofState.commandBuffer,
			);
			if (immediateCommand) {
	...
	private tryParseDiffPatch(
		...
		if (malformedPatchMatch) {
			const eofMarker = malformedPatchMatch[1]
				.replace(/['"]/g, '')
				.trim();
	...
	private processAccumulatedBetweenFilesContent(
		...
		const installCommands = extractCommands(
