...
`;
	...
	private processXmlContent(
		...
		if (
			state.contentBuffer.length > (state as any).maxBufferSize ||
			10000
		) {
	...
	private processXmlTag(
		...
		if (type === 'opening') {
			...
			return this.handleSelfClosingTag(
				tagName,
				attributes,
				state,
				callbacks,
	...
	private handlePartialContent(
		...
		if (state.currentElement && state.contentBuffer.length > 50) {
			...
			if (!hasPartialTag) {
				...
				this.addContentToCurrentElement(
					state,
					state.contentBuffer,
					callbacks,
				);
				...
				if (lastTagIndex > 0) {
					const safeContent = state.contentBuffer.substring(
						0,
						lastTagIndex,
					);
					this.addContentToCurrentElement(
					...
					state.contentBuffer =
						state.contentBuffer.substring(lastTagIndex);
				}
			}
		}
	}
