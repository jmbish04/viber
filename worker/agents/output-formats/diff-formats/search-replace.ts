...
function parseSearchReplaceDiff(diffContent: string): ParseResult {
	...
	function completeBlock(
		...
		if (block.search.length === 0) {
			errors.push(
				`Block at line ${block.startLine + 1}: Empty SEARCH section`,
			);
			return;
		}
...
function findFuzzyMatch(
	...
	const matchLines = contentLines.slice(
		bestMatch.startLine,
		bestMatch.endLine,
	);
	...
	switch (strategy) {
		...
		case MatchingStrategy.INDENTATION_PRESERVING: {
			...
			if (searchPattern.pattern) {
				count =
					contentPattern.pattern.split(searchPattern.pattern).length -
					1;
			}
			break;
		}
...
export function applyDiff(
	...
	try {
		...
		const { blocks, errors: parseErrors } =
			parseSearchReplaceDiff(diffContent);
		...
		if (
			result.results.blocksApplied === 0 &&
			result.results.blocksTotal > 0
		) {
