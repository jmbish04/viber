...
function normalizeIndentation(lines: string[]): {
	...
	for (const line of nonEmptyLines.slice(1)) {
		...
		let i = 0;
		while (
			i < Math.min(commonPrefix.length, prefix.length) &&
 ...
 */
function tryWithWhitespaceNormalization(
	content: string,
	before: string[],
	after: string[],
): string | null {
	...
	for (let i = 0; i <= contentLines.length - before.length; i++) {
		...
		for (let j = 0; j < before.length; j++) {
			const contentLine = normalizeLineWhitespace(
				contentLines[i + j] || '',
			);
		...
		if (matches) {
			...
			const processedAfter = after.map((line, idx) => {
				...
				const refLine =
					originalSlice[Math.min(idx, originalSlice.length - 1)] ||
					originalSlice[0] ||
					'';
 ...
 */
function tryWithIndentationAwareMatching(
	content: string,
	before: string[],
	after: string[],
): string | null {
	...
	for (let i = 0; i <= contentLines.length - before.length; i++) {
		...
		const { normalized: normalizedContent } =
			normalizeIndentation(contentSlice);
		...
		if (matches) {
			...
			const indentedAfter = after.map((afterLine, idx) => {
				...
				const originalIndent =
					indentMap.get(correspondingBeforeLine) ||
 ...
 */
function tryWithNormalizedWhitespace(
	content: string,
	hunk: string[],
): string | null {
	const { before, after } = hunkToBeforeAfter(hunk);
 ...
 */
function createFlexibleWhitespacePattern(
 ...
 */
function trySubHunkDecomposition(
	content: string,
	hunk: string[],
): string | null {
 ...
 */
function applyHunkWithoutRecursion(
	content: string,
	hunk: string[],
): string | null {
...
function applyHunkWithTelemetry(
	...
	// All strategies failed
	monitor.addWarning(
		'All standard strategies failed, attempting fallback strategies',
	);
	return null;
}
...
class DiffSecurityValidator {
	...
	static validateDiff(diff: string): void {
		...
		if (diff.length > PRODUCTION_LIMITS.MAX_DIFF_SIZE) {
			throw new Error(
				`Diff too large: ${diff.length} bytes exceeds ${PRODUCTION_LIMITS.MAX_DIFF_SIZE} byte limit`,
			);
		}
...
class PerformanceMonitor {
	...
	checkTimeout(): void {
		...
		if (elapsed > PRODUCTION_LIMITS.PROCESSING_TIMEOUT) {
			throw new Error(
				`Processing timeout: ${elapsed}ms exceeds ${PRODUCTION_LIMITS.PROCESSING_TIMEOUT}ms limit`,
			);
		}
	}
	...
	incrementIteration(): void {
		...
		if (this.iterationCount > PRODUCTION_LIMITS.MAX_ITERATIONS) {
			throw new Error(
				`Maximum iterations exceeded: ${this.iterationCount}`,
			);
		}
...
export function applyDiff(
	...
	try {
		...
		if (hunksRaw.length > PRODUCTION_LIMITS.MAX_HUNKS) {
			throw new Error(
				`Too many hunks: ${hunksRaw.length} exceeds ${PRODUCTION_LIMITS.MAX_HUNKS} limit`,
			);
		}
		...
		for (let i = 0; i < hunks.length; i++) {
			...
			try {
				const newContent = applyHunkWithTelemetry(
					currentContent,
					hunk,
					telemetry,
					monitor,
				...
				if (newContent !== null) {
					...
					if (newContent.length === 0 && currentContent.length > 0) {
						...
						throw new Error(
							`Hunk #${i + 1} would delete entire file content - aborting for safety`,
						);
					}
					...
					const errorDetails = [
						...
						hunkPreview.substring(0, 500) +
							(hunkPreview.length > 500 ? '...' : ''),
						``,
						`Analysis:`,
