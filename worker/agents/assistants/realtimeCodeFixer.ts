...
import {
	MatchingStrategy,
	FailedBlock,
} from '../output-formats/diff-formats/search-replace';
import {
	AIModels,
	ModelConfig,
	InferenceContext,
...
export class RealtimeCodeFixer extends Assistant<Env> {
	...
	async run(
		...
		try {
			...
			if (
				generatedFile.filePath.endsWith('.css') ||
				generatedFile.filePath.endsWith('.json') ||
				generatedFile.filePath.endsWith('.config.js')
			) {
				this.logger.info(
					`Skipping realtime code fixer for file: ${generatedFile.filePath}`,
				);
...
`),
				...
				searchBlocks =
					fixResult.string.match(/<<<\s+SEARCH/g)?.length ?? 0;
...
`);
	...
	async applyDiffSafely(
		...
		if (!originalContent || !originalDiff?.trim()) {
			this.logger.warn(
				'Empty content or diff provided to applyDiffSafely',
			);
		...
		try {
			...
			for (let attempt = 0; attempt <= maxRetries; attempt++) {
				...
				const searchBlocks =
					currentDiff.match(/<<<\s+SEARCH/g)?.length ?? 0;
				const replaceBlocks =
					currentDiff.match(/\s+REPLACE/g)?.length ?? 0;
...
`,
			...
			this.logger.info(
				`Getting corrected diff from LLM...${failedBlocksText}`,
			);
			...
			const replaceCount = (trimmed.match(/>>>>>>> REPLACE/g) || [])
				.length;
