...
export async function getAgentStub(
...
): Promise<DurableObjectStub<SmartCodeGeneratorAgent>> {
	if (searchInOtherJurisdictions) {
		...
		for (const jurisdiction of jurisdictions) {
			try {
				logger.info(
					`Agent ${agentId} retreiving from jurisdiction ${jurisdiction}`,
				);
				const stub = await getAgentByName<Env, SmartCodeGeneratorAgent>(
					env.CodeGenObject,
					agentId,
					{
						locationHint: 'enam',
				...
				if (isInitialized) {
					logger.info(
						`Agent ${agentId} found in jurisdiction ${jurisdiction}`,
					);
					return stub;
				}
			} catch (error) {
				logger.info(
					`Agent ${agentId} not found in jurisdiction ${jurisdiction}`,
				);
			}
		}
	...
	return getAgentByName<Env, SmartCodeGeneratorAgent>(
		env.CodeGenObject,
		agentId,
		{
			locationHint: 'enam',
...
export async function getTemplateForQuery(
	...
	const selectedTemplate = templatesResponse.templates.find(
		(template) =>
			template.name === analyzeQueryResponse.selectedTemplateName,
	);
	if (!selectedTemplate) {
	...
	const templateDetailsResponse = await sandboxClient.getTemplateDetails(
