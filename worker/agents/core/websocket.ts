...
export function handleWebSocketMessage(
	...
	try {
		logger.info(
			`Received WebSocket message from ${connection.id}: ${message}`,
		);
		...
		switch (parsedMessage.type) {
			...
			case WebSocketMessageRequests.CODE_REVIEW:
				if (agent.isGenerating) {
					sendError(
						connection,
						'Cannot perform code review while generating files',
					);
					return;
				}
				sendToConnection(
						...
						if (
							...
							for (const fileToFix of reviewResult.filesToFix) {
								...
								agent
									...
									.catch((error: unknown) => {
										logger.error(
											`Error regenerating file ${fileToRegenerate.filePath}:`,
											error,
										);
										sendError(
											connection,
											`Error regenerating file: ${error instanceof Error ? error.message : String(error)}`,
										);
									});
							}
						}
			...
			case WebSocketMessageRequests.DEPLOY:
				agent
					...
					.then((deploymentResult) => {
						if (!deploymentResult) {
							logger.error(
								'Failed to deploy to Cloudflare Workers',
							);
							return;
						}
						logger.info(
			...
			case WebSocketMessageRequests.PREVIEW:
				...
				agent
					...
					.then((deploymentResult) => {
						logger.info(
							`Preview deployed successfully!, deploymentResult: ${deploymentResult}`,
						);
					})
					.catch((error: unknown) => {
			...
			case WebSocketMessageRequests.RUNTIME_ERROR_FOUND:
				...
				agent.setState({
					...agent.state,
					clientReportedErrors: parsedMessage.data,
				});
				break;
			case WebSocketMessageRequests.SCREENSHOT_CAPTURED:
				...
				agent
					.saveScreenshotToDatabase(parsedMessage.data)
					.catch((error) => {
						logger.error(
							'Error saving screenshot to database:',
							error,
						);
					});

				logger.info(
					`Screenshot saved to state and database update initiated.`,
				);
				break;
			case WebSocketMessageRequests.STOP_GENERATION:
				// Clear shouldBeGenerating flag when user manually stops
				logger.info(
					'Stopping code generation and clearing shouldBeGenerating flag',
				);
				agent.setState({
					...agent.state,
					shouldBeGenerating: false,
			...
			case WebSocketMessageRequests.USER_SUGGESTION:
				...
				if (!parsedMessage.message) {
					sendError(
						connection,
						'No message provided in user suggestion',
					);
					return;
				}
