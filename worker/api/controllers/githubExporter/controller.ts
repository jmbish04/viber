		...
		try {
			...
			if (stateParam) {
				try {
					...
					this.logger.error(
						'Failed to parse OAuth state parameter',
						error,
					);
				}
			}
			...
			const { userId, purpose, agentId, exportData, returnUrl } =
				parsedState;
			...
			if (purpose === 'repository_export' && exportData) {
				...
				if (!createResult.success || !createResult.repository) {
					this.logger.error(
						'Failed to create repository during export',
						{
				...
				this.logger.info(
					'Repository created successfully, now pushing files',
					{
						userId,
				...
				if (agentId) {
					try {
						...
						this.logger.info(
							'Pushing files to repository via agent',
							{
								agentId,
								repositoryUrl: createResult.repository.html_url,
							},
						...
						const pushResult =
							await agentStub.pushToGitHub(pushRequest);
						...
						if (!pushResult?.success) {
							this.logger.error(
								'Failed to push files to repository',
								{
									...
									agentId,
									repositoryUrl:
										createResult.repository.html_url,
								},
							);
							return Response.redirect(
						...
						this.logger.info(
							'Successfully completed GitHub export with files',
							{
								userId,
								agentId,
								repositoryUrl: createResult.repository.html_url,
					...
					this.logger.warn(
						'No agentId provided - repository created but files not pushed',
						{
							repositoryUrl: createResult.repository.html_url,
						},
					);
