...
export class ModelProvidersController extends BaseController {
	...
	static async getProviders(
		...
		try {
			...
			const providers = await modelProvidersService.getUserProviders(
				user.id,
			);
	...
	static async createProvider(
		...
		try {
			...
			const modelProvidersService = new ModelProvidersService(env);
			const exists = await modelProvidersService.providerExists(
				user.id,
				name,
			);
			if (exists) {
			...
			const provider = await modelProvidersService.createProvider(
				user.id,
				{
					name,
					baseUrl,
	...
	static async updateProvider(
		...
		try {
			...
			const modelProvidersService = new ModelProvidersService(env);
			const secretsService = new SecretsService(env);
			const existingProvider = await modelProvidersService.getProvider(
				user.id,
				providerId,
			);
			if (!existingProvider) {
			...
			const updatedProvider = await modelProvidersService.updateProvider(
				user.id,
				providerId,
				{
					name: updates.name,
	...
	static async deleteProvider(
		...
		try {
			...
			const updated = await modelProvidersService.updateProvider(
				user.id,
				providerId,
				{
					isActive: false,
	...
	static async testProvider(
		...
		try {
			...
			if (validation.data.providerId) {
				...
				const secretValue = await secretsService.getSecretValue(
					user.id,
					provider.secretId,
				);
				if (!secretValue) {
