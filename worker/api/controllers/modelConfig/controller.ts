...
export class ModelConfigController extends BaseController {
	...
	static async getModelConfigs(
		...
		try {
			...
			const configs = await modelConfigService.getUserModelConfigs(
				user.id,
			);
	...
	static async getModelConfig(
		...
		try {
			...
			const defaultConfig =
				modelConfigService.getDefaultConfigs()[agentAction];
	...
	static async updateModelConfig(
		...
		try {
			...
			const bodyResult =
				await ModelConfigController.parseJsonBody(request);
			...
			const validatedData = modelConfigUpdateSchema.parse(
				bodyResult.data,
			);
			...
			if (modelConfig.name || modelConfig.fallbackModel) {
				const userProviderStatus = await getUserProviderStatus(
					user.id,
					env,
				);
				...
				if (modelConfig.fallbackModel) {
					...
					if (!isValidAccess) {
						const provider =
							modelConfig.fallbackModel.split('/')[0];
						return ModelConfigController.createErrorResponse<ModelConfigUpdateData>(
			...
			const modelConfigService = new ModelConfigService(env);
			const updatedConfig =
				await modelConfigService.upsertUserModelConfig(
					user.id,
					agentAction,
					modelConfig,
				);
		...
		try {
			...
			const configToTest: ModelConfig = validatedData.tempConfig
				? {
						...
						...(validatedData.tempConfig.reasoningEffort !=
							null && {
							reasoning_effort:
								validatedData.tempConfig.reasoningEffort,
						}),
						...(validatedData.tempConfig.fallbackModel != null && {
							fallbackModel:
								validatedData.tempConfig.fallbackModel,
						}),
						...(validatedData.tempConfig.providerOverride !=
							null && {
							providerOverride:
								validatedData.tempConfig.providerOverride,
						}),
			...
			if (validatedData.useUserKeys) {
				const userApiKeysMap = await secretsService.getUserBYOKKeysMap(
					user.id,
				);
	...
	static async resetAllConfigs(
		...
		try {
			...
			const modelConfigService = new ModelConfigService(env);
			const resetCount = await modelConfigService.resetAllUserConfigs(
				user.id,
			);
			...
			this.logger.error(
				'Error resetting all model configurations:',
				error,
			);
