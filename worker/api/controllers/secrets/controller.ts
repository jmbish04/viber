...
export class SecretsController extends BaseController {
	...
	): Promise<ControllerResponse<ApiResponse<SecretsData>>> {
		try {
			...
			return SecretsController.createErrorResponse<SecretsData>(
				'Failed to get all user secrets',
				500,
			);
		}
	}
	...
	static async storeSecret(
		...
		_ctx: ExecutionContext,
		context: RouteContext,
	): Promise<ControllerResponse<ApiResponse<SecretStoreData>>> {
		try {
			const user = context.user!;
			...
			if (!bodyResult.success) {
				return bodyResult.response! as ControllerResponse<
					ApiResponse<SecretStoreData>
				>;
			}

			const { templateId, name, envVarName, value, description } =
				bodyResult.data!;

			// Validate required fields
			if (!value) {
				return SecretsController.createErrorResponse<SecretStoreData>(
			...
			if (templateId) {
				...
				if (
					template.validation &&
					!new RegExp(template.validation).test(value)
				) {
					return SecretsController.createErrorResponse<SecretStoreData>(
						`Invalid format for ${template.displayName}. Expected format: ${template.placeholder}`,
						400,
					);
				}
				...
				if (!name || !envVarName) {
					return SecretsController.createErrorResponse<SecretStoreData>(
						'Missing required fields for custom secret: name, envVarName',
						400,
					);
				}
				...
				if (!/^[A-Z][A-Z0-9_]*$/.test(envVarName)) {
					return SecretsController.createErrorResponse<SecretStoreData>(
						'Environment variable name must be uppercase and contain only letters, numbers, and underscores',
						400,
					);
				}
			...
			const secretsService = new SecretsService(env);
			const storedSecret = await secretsService.storeSecret(
				user.id,
				secretData,
			);
	...
	static async deleteSecret(
		...
		_ctx: ExecutionContext,
		context: RouteContext,
	): Promise<ControllerResponse<ApiResponse<SecretDeleteData>>> {
		try {
			const user = context.user!;
			...
			if (!secretId) {
				return SecretsController.createErrorResponse<SecretDeleteData>(
	...
	static async toggleSecret(
		...
		try {
			...
			const secretsService = new SecretsService(env);
			const toggledSecret = await secretsService.toggleSecretActiveStatus(
				user.id,
				secretId,
			);
			...
			return SecretsController.createErrorResponse<SecretStoreData>(
				'Failed to toggle secret status',
				500,
			);
		}
	}
