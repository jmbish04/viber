...
export class CodingAgentController extends BaseController {
	...
	static async startCodeGeneration(
		...
		try {
			...
			this.logger.info(
				`Initialized inference context for user ${user.id}`,
				{
			...
			const agentPromise = agentInstance.initialize(
				{
					...
					frameworks:
						body.frameworks || defaultCodeGenArgs.frameworks,
					hostname,
					inferenceContext,
					images: body.images,
	...
	static async handleWebSocketConnection(
		...
		try {
			...
			this.logger.info(
				`WebSocket connection request for chat: ${chatId}`,
			);
			...
			try {
				...
				this.logger.info(
					`Successfully got agent instance for chat: ${chatId}`,
				);
				...
				this.logger.error(
					`Failed to get agent instance with ID ${chatId}:`,
					error,
				);
