...
export const users = sqliteTable(
	'users',
	{
		...
		isSuspended: integer('is_suspended', { mode: 'boolean' }).default(
			false,
		),
...
export const stars = sqliteTable(
	'stars',
	{
		...
		appId: text('app_id')
			.notNull()
			.references(() => apps.id, { onDelete: 'cascade' }),
		starredAt: integer('starred_at', { mode: 'timestamp' }).default(
			sql`CURRENT_TIMESTAMP`,
		),
	},
	(table) => ({
...
export const appViews = sqliteTable(
	'app_views',
	{
		...
		// Timing
		viewedAt: integer('viewed_at', { mode: 'timestamp' }).default(
			sql`CURRENT_TIMESTAMP`,
		),
