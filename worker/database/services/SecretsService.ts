...
export class SecretsService extends BaseService {
	...
	async encryptSecret(
		value: string,
	): Promise<{ encryptedValue: string; keyPreview: string }> {
		try {
			...
			const keyMaterial = await this.deriveKey(
				this.env.SECRETS_ENCRYPTION_KEY,
				salt,
			);
		...
		try {
			...
			for (const template of byokTemplates) {
				const secret = secrets.find(
					(s) => s.secretType === template.envVarName,
				);

				if (secret) {
					try {
						const decryptedKey = await this.decryptSecret(
							secret.encryptedValue,
						);
						keyMap.set(template.provider, decryptedKey);
					} catch (error) {
						this.logger.error(
							`Failed to decrypt BYOK key for provider ${template.provider}:`,
							error,
						);
					}
				}
			...
			this.logger.info(
				`Loaded ${keyMap.size} BYOK API keys from secrets system`,
				{ userId },
			);
			return keyMap;
		} catch (error) {
	 ...
	 */
	async toggleSecretActiveStatus(
		_userId: string,
		_secretId: string,
	): Promise<EncryptedSecret> {
		// DISABLED: BYOK Disabled for security reasons
