	...
	async getUserModelConfigs(
		...
		for (const [actionKey, defaultConfig] of Object.entries(AGENT_CONFIG)) {
			...
			if (userConfig) {
				...
				result[actionKey] = {
					name:
						(userConfig.modelName as AIModels) ??
						defaultConfig.name,
					max_tokens:
						userConfig.maxTokens ?? defaultConfig.max_tokens,
					temperature:
						userConfig.temperature !== null
							? userConfig.temperature
							: defaultConfig.temperature,
					reasoning_effort:
						this.castToReasoningEffort(
							userConfig.reasoningEffort,
						) ?? defaultConfig.reasoning_effort,
					fallbackModel:
						(userConfig.fallbackModel as AIModels) ??
			...
			return {
				...
				temperature:
					config.temperature !== null
						...
						: defaultConfig.temperature,
				reasoning_effort:
					this.castToReasoningEffort(config.reasoningEffort) ??
					defaultConfig.reasoning_effort,
				fallbackModel:
					(config.fallbackModel as AIModels) ??
					defaultConfig.fallbackModel,
				isUserOverride: true,
