...
export class AuthService extends BaseService {
	...
	async register(
		...
		try {
			...
			const { accessToken, session } =
				await this.sessionService.createSession(userId, request);
	...
	async getOAuthAuthorizationUrl(
		...
		if (intendedRedirectUrl) {
			validatedRedirectUrl = this.validateRedirectUrl(
				intendedRedirectUrl,
				request,
			);
		}
	...
	async handleOAuthCallback(
		...
		try {
			...
			// Get user info
			const oauthUserInfo = await oauthProvider.getUserInfo(
				tokens.accessToken,
			);
	...
	private async findOrCreateOAuthUser(
		...
		if (!user) {
			...
			await this.database.insert(schema.users).values({
				...
				displayName:
					oauthUserInfo.name || oauthUserInfo.email.split('@')[0],
			...
			await this.database
				...
				.set({
					...
					providerId: oauthUserInfo.id,
					emailVerified:
						oauthUserInfo.emailVerified || user.emailVerified,
					updatedAt: new Date(),
				})
	...
	private validateRedirectUrl(
		...
		try {
			...
			if (
				authPaths.some((path) =>
					redirectUrlObj.pathname.startsWith(path),
				)
			) {
			...
			logger.warn('Invalid OAuth redirect URL format', {
				redirectUrl,
				error,
			});
			return null;
	 ...
	 */
	private async generateAndStoreVerificationOtp(
		email: string,
	): Promise<void> {
	...
	async verifyEmailWithOtp(
		...
		try {
			...
			// Verify OTP
			const otpValid = await this.passwordService.verify(
				otp,
				storedOtp.otp,
			);
			if (!otpValid) {
